                    ...
                    try:
                        ...
                        if category_name not in category_cache:
                            category, created = Category.objects.get_or_create(
                                name=category_name,
                                defaults={'description': f'Categoría de {category_name}'}
                            )
                        ...
                        product_name = row['display name'].strip()[:100]
                        if not product_name:
                        ...
                        # Generar slug único
                        base_slug = slugify(product_name)[:90]
                        slug = base_slug
                        counter = 1
                        ...
                        with transaction.atomic():
                            product = Product.objects.create(
                                ...
                                category=category,
                                status='ACTIVE',
                                is_featured=(index % 20 == 0)  # 5% featured
                            )
